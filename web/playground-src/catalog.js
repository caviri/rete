...
window.RETE_PLAYGROUND_CATALOG = {
  ...
  examples: {
    "gotriple": [
      {"family": "Summary", "label": "What's in the graph — by type", "view": "table", "cols": {"class": "Type", "n": "Count"},
       "tip": "The three entity types: 2.71M gtr:Document (distinct SSH publications, deduplicated by DOI/id from 6.07M discipline-placements), ~2,600 gtr:Subject (the shared SSH-LCSH authority concepts), and the 27 gtr:Discipline concepts from the embedded ontology.\nThe Schema tab shows the type pyramid read straight from the file.",
       "q": "PREFIX gtr: <https://w3id.org/rete/gotriple#>\nSELECT ?class (COUNT(*) AS ?n) WHERE {\n  ?s a ?class .\n  FILTER(STRSTARTS(STR(?class), STR(gtr:)))\n} GROUP BY ?class ORDER BY DESC(?n)"},
      {"family": "Aggregate", "label": "Publications per discipline", "view": "table", "cols": {"disc": "Discipline", "n": "Publications"},
       "tip": "Counting gtr:inDiscipline edges — a paper can be in several disciplines, so these sum to more than the 2.71M distinct documents. History, Sociology, Environmental studies and Linguistics lead.\nEach discipline IRI (gtr:discipline-hist …) carries an skos:prefLabel from the embedded ontology.",
       "q": "PREFIX gtr: <https://w3id.org/rete/gotriple#>\nPREFIX skos: <http://www.w3.org/2004/02/skos/core#>\nSELECT ?label (COUNT(?d) AS ?n) WHERE {\n  ?d gtr:inDiscipline ?disc .\n  ?disc skos:prefLabel ?label .\n} GROUP BY ?disc ?label ORDER BY DESC(?n)"},
      {"family": "Select", "label": "Publications about a subject (SSH-LCSH)", "view": "table", "cols": {"title": "Publication"},
       "tip": "gtr:knowsAbout links a document to a linked SSH-LCSH subject authority (a shared node minted at its semantics.gr URI, so it dedupes across the whole corpus into a real subject graph). This lists documents about 'Education' (sh85040989), which links ~288k documents.\nSwap the subject IRI — find one via its rdfs:label — to pivot the corpus by topic.",
       "q": "PREFIX gtr: <https://w3id.org/rete/gotriple#>\nPREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>\nSELECT ?title WHERE {\n  ?d gtr:knowsAbout <http://semantics.gr/authorities/SSH-LCSH/sh85040989> ;\n     rdfs:label ?title .\n} LIMIT 50"},
      {"family": "Select", "label": "Open-access law papers with a full-text link", "view": "table", "cols": {"title": "Paper", "url": "Full text"},
       "tip": "A selective slice: documents in one discipline (Law) that are open-access and carry a working full-text URL. Each ?url opens the PDF.\nChange gtr:discipline-droit to any discipline (gtr:discipline-socio, -hist, -psy…) or drop the access filter.",
       "q": "PREFIX gtr: <https://w3id.org/rete/gotriple#>\nSELECT ?title ?url WHERE {\n  ?d gtr:inDiscipline gtr:discipline-droit ;\n     gtr:conditionsOfAccess \"acr_open-access\" ;\n     gtr:fullTextUrl ?url ;\n     gtr:title ?title .\n} LIMIT 40"},
      {"family": "Construct", "label": "A publication's subjects, as a graph", "view": "graph",
       "tip": "Switch Output to Graph. One document at the centre with its linked SSH-LCSH subjects fanning out (gtr:knowsAbout) — how a paper is placed in the subject network.\nBecause the document sits at its canonical DOI IRI, this same node is where it would join the DataCite / Zenodo / OpenAIRE graphs. Change the DOI to explore another paper.",
       "q": "PREFIX gtr: <https://w3id.org/rete/gotriple#>\nPREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>\nCONSTRUCT {\n  ?d gtr:knowsAbout ?s .\n  ?d rdfs:label ?dl .\n  ?s rdfs:label ?sl .\n} WHERE {\n  VALUES ?d { <https://doi.org/10.33899/radab.1972.166508> }\n  OPTIONAL { ?d rdfs:label ?dl }\n  ?d gtr:knowsAbout ?s .\n  OPTIONAL { ?s rdfs:label ?sl }\n}"}
    ],
    "zenodo-records": [
