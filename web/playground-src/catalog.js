...
window.RETE_PLAYGROUND_CATALOG = {
  ...
  datasetMeta: {
    ...
    "mtg":                   { triples: "920 K", size: "9.8 MB", license: "Card data MIT (MTGJSON); images © WotC via Scryfall", source: "https://mtgjson.com/", provenance: "Built 2026-07-13 from MTGJSON AtomicCards (Oracle-level, 34,633 cards) + Scryfall oracle-cards bulk for card images. scripts/mtg/build_mtg_graph.py maps each card to its colors, card types, subtypes, supertypes, keywords, per-format legalities, sets and Oracle text; card types are OWL classes (rdfs:subClassOf mtg:Card) with each card rdf:type its types. rete build --pyramid-algo types --text-index --card. Vocabulary inspired by cmdoret/mtg_ontology (GPL-3.0); the card-semantics layer of a semantic MTG engine." },
