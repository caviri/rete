...
window.RETE_PLAYGROUND_CATALOG = {
  ...
  datasets: [
    ...
    {"key": "arxiu", "kind": "remote-lazy", "url": "https://data.graphplaza.com/arxiu/arxiu.rete", "label": "arxiu.rete — Arxius en Línia: Catalan national archives, with scanned documents (sample)", "description": "A working SAMPLE of the Generalitat de Catalunya's online archives (Arxius en Línia) — 500 archival units across 36 archives and 64 fonds, each with its holding archive, fonds, reference code, dates, description AND its scanned document image. Every scan is shown as a downsized WebP (~180 KB, ~15% of the ~1.5 MB original) mirrored to R2 so it renders inline in query results, while the original full-res JPEG on Gencat's server is kept as arx:originalImage — the source of record. The full archive behind the same open API is enormous — ~5.9 million records and ~16.3 million scanned images (~1.5 TB) — so this is a first slice proving the harvest + WebP pipeline before scaling. Modelled with schema.org archival vocabulary (schema:ArchiveComponent held by an ArchiveOrganization, part of a Collection/fonds) on w3id.org/rete. Descriptions are openly reusable (Llicència oberta d'ús d'informació / Llei 37/2007, Generalitat de Catalunya, with attribution); the reservat rights flag is respected — reserved images are linked, never re-hosted."},
  ],
  ...
  datasetMeta: {
    ...
    "arxiu":                 { triples: "7.2 K", size: "266 KB + 452 WebP", license: "Descriptions: Llicència oberta / CC0 (Llei 37/2007); images © holding archives", source: "https://arxiusenlinia.cultura.gencat.cat/", provenance: "Harvested 2026-07-13 (sample of 500 records with images) from the open backend API (backend.arxiusenlinia.cultura.gencat.cat/unitat/search/advanced, mostrar=AMB_FITXER). scripts/arxiu/harvest_and_mirror.py pages the search + mirrors each non-reserved image as a downsized WebP (max 1400px, q80, ~15% of original) to R2 while keeping the original Gencat link; scripts/arxiu/build_arxiu_graph.py → schema.org archival N-Triples → rete build --pyramid-algo types --text-index --card. Full archive is ~5.9M records / ~16.3M images (~1.5 TB)." },
  ...
  datasetExtra: {
    ...
    "arxiu":                 { icon: "🗃️", tags: ["archives","Catalonia","Generalitat","manuscripts","scanned documents","WebP","IIIF-free","schema.org","open data","sample"] },
  ...
  examples: {
    ...
    "arxiu": [
      {"family": "Summary", "label": "🏛 Archives in this sample", "view": "table", "cols": {"archive": "Archive", "units": "Units"}, "tip": "The sample spans 36 Catalan archives (the Arxiu Nacional, the comarcal archives, the historical provincial archives …), each holding a share of the 500 units.\nEvery unit is a schema:ArchiveComponent linked to its holding archive.", "q": "PREFIX schema: <http://schema.org/>\nSELECT ?archive (COUNT(?u) AS ?units) WHERE {\n  ?u schema:holdingArchive ?a . ?a schema:name ?archive .\n} GROUP BY ?archive ORDER BY DESC(?units)"},
      {"family": "Media", "label": "🖼 Scanned documents (gallery)", "view": "table", "cols": {"image": "Scan", "title": "Title", "archive": "Archive"}, "tip": "The scanned documents themselves — each schema:image is the downsized WebP mirrored to R2 (rendered here), ~15% the size of Gencat's original JPEG. Many are medieval parish and notarial records.\nDrop the LIMIT to browse the whole sample.", "q": "PREFIX schema: <http://schema.org/>\nSELECT ?image ?title ?archive WHERE {\n  ?u schema:image ?image ; schema:name ?title ; schema:holdingArchive ?a .\n  ?a schema:name ?archive .\n} LIMIT 30"},
      ...
      {"family": "Search", "label": "🔎 Documents mentioning Barcelona", "view": "table", "cols": {"image": "Scan", "title": "Title", "desc": "Description"}, "tip": "Full-text over the descriptions (a TEXT_INDEX is built in): units whose description mentions “Barcelona”, with their scan.\nChange the word to search the catalogue by content.", "q": "PREFIX schema: <http://schema.org/>\nSELECT ?image ?title ?desc WHERE {\n  ?u schema:description ?desc ; schema:name ?title .\n  OPTIONAL { ?u schema:image ?image }\n  FILTER(CONTAINS(LCASE(?desc), \"barcelona\"))\n} LIMIT 24"},
