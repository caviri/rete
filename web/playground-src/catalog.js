...
window.RETE_PLAYGROUND_CATALOG = {
  ...
  datasetMeta: {
    ...
    "arxiu":                 { triples: "2.2 M", size: "17.4 MB", license: "Descriptions: Llicència oberta / CC0 (Llei 37/2007); images © holding archives", source: "https://arxiusenlinia.cultura.gencat.cat/", provenance: "Harvested 2026-07-13, whole archive by whole archive, from the open backend API (backend.arxiusenlinia.cultura.gencat.cat/unitat/search/advanced, mostrar=AMB_FITXER, sort=id for stable complete pagination): Arxiu Històric de Tarragona (330, 7,401 units) + Arxiu Comarcal del Baix Llobregat (50, 53,644) + Arxiu Comarcal de l'Anoia (10, 137,054) = 198,099 units, 207 fonds, 8,634 WebP scans (non-reserved photos + PDF first-pages rendered via PyMuPDF). scripts/arxiu/harvest_and_mirror.py pages each archive + mirrors non-reserved images as downsized WebP (max 1400px, q80) to R2 while keeping the original Gencat link; scripts/arxiu/build_arxiu_graph.py (accumulative over records_*.jsonl, deduped) → schema.org archival N-Triples → rete build --pyramid-algo types --text-index --card. Full archive ~1.43M image-records across ~65 archives." },
  ...
  examples: {
    ...
    "arxiu": [
      ...
      {"family": "Select", "label": "🗺 Files about the Conca de Barberà", "view": "cards", "cols": {"image": "Scan", "title": "Title", "description": "Description", "original": "Original"},
       ...
       "q": "PREFIX schema: <http://schema.org/>\nPREFIX arx: <https://w3id.org/rete/arxiu#>\nSELECT (MIN(STR(?img)) AS ?image) ?title (SAMPLE(?d) AS ?description) (SAMPLE(?o) AS ?original) WHERE {\n  ?doc schema:name ?title ;\n       schema:image ?img .\n  FILTER(CONTAINS(LCASE(STR(?title)), \"conca de barber\"))\n  OPTIONAL { ?doc schema:description ?d }\n  OPTIONAL { ?doc arx:originalImage ?o }\n} GROUP BY ?doc ?title LIMIT 40"},
