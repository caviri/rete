...
window.RETE_PLAYGROUND_CATALOG = {
  ...
  // it's ~3.5x faster than the old HF Space. One folder per dataset: <base>/<key>/<key>.rete.
  ...
  datasetMeta: {
    ...
    "wikidata-zenodo":       { triples: null,      size: "1.49 GB", license: "CC0",                  source: "https://zenodo.org/records/21168821",                      provenance: "The same Wikidata cross-section as wikidata-1GB (scripts/wikidata_parquet_to_nt.py → rete build), published to Zenodo as a citable research artifact (record 21168821, DOI 10.5281/zenodo.21168821) and range-queried STRAIGHT from the Zenodo record — no special server, any HTTP-range + CORS host works. Zenodo hides Content-Range, so rete's reader learns the file length via a HEAD request (fix ee7a5f9). Remote-lazy." },
  ...
  examples: {
    ...
    "bcn": [
      ...
      {"family": "Select", "label": "🔎 Full-text search — the vanished Somorrostro beach", "view": "cards", "cols": {"title": "Record", "summary": "Description", "archive": "Archive", "url": "Open ↗"}, "tip": "The whole 529k-record catalogue is full-text indexed over titles and scope summaries. This finds records mentioning the Somorrostro, the shanty-beach cleared for the 1992 Olympics — change the CONTAINS term to search anything (a street, a person, an event).\n?url opens the record's /detail page on the archive's own deep-zoom viewer.", "q": "PREFIX schema: <http://schema.org/>\nSELECT ?title ?summary ?archive ?url WHERE {\n  ?u schema:name ?title ;\n     schema:description ?summary ;\n     schema:holdingArchive ?a ;\n     schema:url ?url .\n  ?a schema:name ?archive .\n  FILTER(CONTAINS(?summary, \"Somorrostro\"))\n} LIMIT 40"},
      {"family": "Select", "label": "🗂 Biggest digital objects, by exact byte size", "view": "table", "cols": {"file": "File", "fmt": "Format", "bytes": "Bytes", "rec": "In record"}, "tip": "597,138 digital objects — mostly scanned photographs (image/jpeg), plus PDFs — each carry their format and exact byte size (schema:contentSize). Biggest first.\nThe object itself opens on the source viewer via the record's /detail page (see the search example); here we rank the raw media.", "q": "PREFIX schema: <http://schema.org/>\nSELECT ?file ?fmt ?bytes ?rec WHERE {\n  ?u schema:associatedMedia ?m ; schema:name ?rec .\n  ?m schema:name ?file ; schema:encodingFormat ?fmt ; schema:contentSize ?bytes .\n} ORDER BY DESC(?bytes) LIMIT 50"},
      {"family": "Select", "label": "📷 Digitised photographs (Arxiu Fotogràfic) — open on the viewer", "view": "table", "cols": {"title": "Photograph", "fmt": "Format", "url": "Open ↗"}, "tip": "Records from the Arxiu Fotogràfic de Barcelona (AFB) that are digitised, each with its image object's format and its /detail page on the archive's OpenSeadragon deep-zoom viewer.\nThe images live behind that viewer (session-gated), so the graph links the record page rather than embedding the pixels.", "q": "PREFIX schema: <http://schema.org/>\nPREFIX bcn: <https://w3id.org/rete/bcn#>\nSELECT ?title ?fmt ?url WHERE {\n  ?u bcn:digitized true ;\n     schema:holdingArchive <https://w3id.org/rete/bcn/archive/AFB> ;\n     schema:name ?title ;\n     schema:url ?url ;\n     schema:associatedMedia ?m .\n  ?m schema:encodingFormat ?fmt .\n} LIMIT 40"}
    ],
    "vidy": [
