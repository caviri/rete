...
window.RETE_PLAYGROUND_CATALOG = {
  ...
  datasetMeta: {
    ...
    "bioexplora":            { triples: "7.19 M",  size: "43 MB",   license: "CC BY 4.0 (specimens/images; audio CC BY-NC-ND)", source: "https://www.bioexplora.cat", provenance: "The 6 MCNB Darwin Core archives from GBIF/ipt.gbif.es (207,163 specimens, real dwc: term IRIs) + 13,543 IIIF images (iiif/portraitMedia, CORS-open, render inline) + 43,826 GeoSPARQL points + 672 type specimens; plus 824 Atles-osteologic 3D scans (Sketchfab account laboratorinatura) — 62 of them mirrored to the bucket as Draco+WebP .glb and rendered INLINE via p:mesh (scripts/bioexplora_sketchfab.sh; the rest link out via p:sketchfab) — and 173 Xeno-canto audio recordings (Eloisa Matheu, CC BY-NC-ND, not MCNB). Built KEYLESS by scripts/bioexplora_to_nt.py + rete build --pyramid-algo types --text-index --card. Remote-lazy. CC BY 4.0 (MCNB) except the audio." },
...
SELECT ?year (COUNT(*) AS ?territories) WHERE {
  ?t ex:year ?year
    ...
    "bioexplora": [
      {"family": "Summary", "label": "The collection at a glance", "view": "table", "tip": "The museum's six collections by specimen count: arthropods (MCNB-Art, 82,900) and molluscs (MCNB-Malac, 50,083) dominate, then paleontology (MGB), vertebrates (MCNB-Cord), the tissue bank and general zoology. 207,163 Darwin Core specimens in all.", "q": "PREFIX dwc: <http://rs.tdwg.org/dwc/terms/>\nSELECT ?collection (COUNT(*) AS ?specimens) WHERE {\n  ?s dwc:collectionCode ?collection .\n} GROUP BY ?collection ORDER BY DESC(?specimens)"},
      {"family": "Select", "label": "Specimens you can SEE (photos)", "view": "table", "tip": "13,543 specimens are photographed. The ?photo column renders INLINE in the table — molluscs, crustaceans, insects, fossils. Images are mirrored to the bucket as fast WebP (p:preview, ~10-40 KB) and also kept as the museum's source link (p:image). CC BY 4.0, credit the Museu de Ciencies Naturals de Barcelona.", "q": "PREFIX dwc: <http://rs.tdwg.org/dwc/terms/>\nPREFIX p: <https://bioexplora.cat/prop/>\nSELECT ?species ?photo WHERE {\n  ?s dwc:scientificName ?species ; p:image ?photo .\n} LIMIT 40"},
      {"family": "Path", "label": "See it, hear it, place it — photo + sound + map", "view": "table", "tip": "One row, four media: each PHOTOGRAPHED, GEOREFERENCED specimen joined to a Xeno-canto RECORDING of the SAME species (on dwc:scientificName). A bird like the Great Crested Grebe shows its specimen photo, its call (inline player) and where it was collected (a mini-map) — all together. Stay in Table view to see all four at once.", "q": "PREFIX dwc: <http://rs.tdwg.org/dwc/terms/>\nPREFIX p: <https://bioexplora.cat/prop/>\nPREFIX geo: <http://www.opengis.net/ont/geosparql#>\nSELECT ?species ?photo ?sound ?location WHERE {\n  ?s dwc:scientificName ?species ; p:image ?photo ; geo:asWKT ?location .\n  ?r dwc:scientificName ?species ; p:audio ?sound .\n} LIMIT 24"},
      {"family": "Aggregate", "label": "Map of the collection", "view": "map", "tip": "43,826 specimens are georeferenced (GeoSPARQL points). Switch Output -> Map to see where the holdings were collected across the world — Catalonia and Spain dominate, with material from Morocco, Mexico, France and beyond. Shows up to 3,000 points.", "q": "PREFIX dwc: <http://rs.tdwg.org/dwc/terms/>\nPREFIX geo: <http://www.opengis.net/ont/geosparql#>\nSELECT ?species ?point WHERE {\n  ?s dwc:scientificName ?species ; geo:asWKT ?point .\n} LIMIT 3000"},
      {"family": "Select", "label": "Type specimens (the holotypes)", "view": "table", "tip": "672 name-bearing TYPE specimens — the single most scientifically important objects in any natural-history collection, because each one anchors a species name. ?type is the kind (Holotype, Paratype, Syntype...), ?cat the catalogue number.", "q": "PREFIX dwc: <http://rs.tdwg.org/dwc/terms/>\nSELECT ?species ?type ?cat WHERE {\n  ?s dwc:typeStatus ?type ; dwc:scientificName ?species ; dwc:catalogNumber ?cat .\n} LIMIT 40"},
      {"family": "Select", "label": "3D skull scans you can rotate (Atles osteologic)", "view": "table", "tip": "Skull and bone 3D scans from the museum's Atles osteologic. The ?mesh column is a Draco-compressed .glb streamed from the bucket and rendered INLINE — click 🧊 3D, then drag to turn the actual cranium of a hyena, fox or bird. 62 models stream inline so far (mirrored + compressed from Sketchfab account laboratorinatura); the rest link out via p:sketchfab. CC BY.", "q": "PREFIX class: <https://bioexplora.cat/class/>\nPREFIX p: <https://bioexplora.cat/prop/>\nPREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>\nSELECT ?model ?mesh WHERE {\n  ?m a class:Model3D ; rdfs:label ?model ; p:mesh ?mesh .\n} LIMIT 40"},
      {"family": "Select", "label": "Nature sounds (the fonoteca)", "view": "table", "tip": "173 nature sound recordings that PLAY inline (the ?audio column is an audio player) — owls, warblers, frogs and more across 100 species. Recorded by Eloisa Matheu, the museum's sound technician, via Xeno-canto. NOTE: these are CC BY-NC-ND by Eloisa Matheu / Xeno-canto, NOT attributable to the MCNB.", "q": "PREFIX class: <https://bioexplora.cat/class/>\nPREFIX p: <https://bioexplora.cat/prop/>\nPREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>\nSELECT ?species ?audio WHERE {\n  ?r a class:Recording ; rdfs:label ?species ; p:audio ?audio .\n} LIMIT 40"},
      {"family": "Path", "label": "Longhorn beetles, and where they were found", "view": "table", "tip": "A taxonomic + geographic slice: every Cerambycidae (longhorn beetle) specimen with its species, catalogue number and collection locality. Change 'Cerambycidae' to any family — Helicidae (snails), Muridae (rodents), Formicidae (ants)...", "q": "PREFIX dwc: <http://rs.tdwg.org/dwc/terms/>\nSELECT ?species ?cat ?locality WHERE {\n  ?s dwc:family \"Cerambycidae\" ; dwc:scientificName ?species ; dwc:catalogNumber ?cat .\n  OPTIONAL { ?s dwc:locality ?locality }\n} LIMIT 40"}
    ],
    "smithsonian3d": [
