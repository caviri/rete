...
window.RETE_PLAYGROUND_CATALOG = {
  ...
  examples: {
    "bcul": [
      {"family": "Select", "label": "What's held at the Cantonal Archives?", "view": "table", "cols": {"title": "Title", "loc": "Shelving location", "call": "Call no.", "avail": "Availability"}, "tip": "Every book resolves to WHERE it physically lives. This lists items held at the Archives cantonales vaudoises with shelving location, call number and live availability.\nSwap \"Archives cantonales\" for \"BCUL site Riponne\", \"Musée des beaux-arts\", \"BCUL site Unithèque\", … to see any of the ~dozens of Vaud sites.", "q": "PREFIX schema: <http://schema.org/>\nPREFIX b: <https://data.bcu-lausanne.ch/>\nSELECT ?title ?loc ?call ?avail WHERE {\n  ?lib schema:name \"Archives cantonales\" .\n  ?it b:heldAt ?lib ;\n      schema:name ?title ;\n      b:shelvingLocation ?loc ;\n      b:callNumber ?call ;\n      b:availability ?avail .\n} LIMIT 100"},
      {"family": "Select", "label": "Medieval manuscripts you can page through (IIIF)", "view": "table", "cols": {"title": "Manuscript", "date": "From", "manifest": "IIIF"}, "tip": "BCUL's 15 medieval manuscripts on e-codices, oldest first — Iordanes' Getica (c. 800), Ovid's Metamorphoses (c. 1100), the Roman de la Rose.\n?manifest is a IIIF manifest: open it to leaf through the actual folios.", "q": "PREFIX schema: <http://schema.org/>\nPREFIX b: <https://data.bcu-lausanne.ch/>\nSELECT ?title ?date ?manifest WHERE {\n  ?m a schema:Manuscript ;\n     schema:name ?title ;\n     schema:startDate ?date ;\n     b:iiifManifest ?manifest .\n} ORDER BY ?date LIMIT 15"},
      {"family": "Select", "label": "Digitized heritage with cover thumbnails (Patrinum)", "view": "table", "cols": {"title": "Title", "thumb": "Cover", "url": "Open"}, "tip": "Patrinum is BCU's own digitized heritage — rare books, iconography, the Fonds Roud.\n?thumb renders inline (334k items have one); ?url opens the full digitized object on patrinum.ch.", "q": "PREFIX schema: <http://schema.org/>\nSELECT ?title ?thumb ?url WHERE {\n  ?it schema:isPartOf <https://data.bcu-lausanne.ch/source/patrinum> ;\n      schema:name ?title ;\n      schema:thumbnailUrl ?thumb ;\n      schema:url ?url .\n} LIMIT 48"},
      ...
      {"family": "Aggregate", "label": "Most prolific authors & bodies in the collection", "view": "table", "cols": {"name": "Author / body", "n": "# works"}, "tip": "The twin links every work to shared author / organisation nodes, so you can rank them: the OECD, IMF and UN dominate the modern catalogue, with the CTLO (medieval Latin texts) close behind.\nAn aggregate over ~2.8M creator edges.", "q": "PREFIX schema: <http://schema.org/>\nSELECT ?name (COUNT(?w) AS ?n) WHERE {\n  ?w schema:creator ?a .\n  ?a schema:name ?name .\n} GROUP BY ?name ORDER BY DESC(?n) LIMIT 25"}
    ],
    "fuero_juzgo": [
