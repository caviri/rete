...
window.RETE_PLAYGROUND_CATALOG = {
  ...
  datasetMeta: {
    ...
    "lineara":               { triples: 38217,     size: "354 KB",  license: "No explicit license (scholarly; attribute GORILA/Douros/mwenge/INSCRIBE)", source: "https://lineara.xyz · 3D: https://www.inscribercproject.com", provenance: "The LinearA Explorer corpus (github.com/mwenge/lineara.xyz, LinearAInscriptions.js — text after GORILA/Godart-Olivier + George Douros's tabulation) converted to RDF by scripts/lineara_to_nt.js: Inscription → site/scribe/support/period + Linear A transcription + Latin transliteration, plus the sign/word layer (Inscription → Word → Sign and Inscription → Sign). scripts/lineara_inscribe.js then adds 91 prop:model3d LINKS to the ERC INSCRIBE project's 3D viewer pages (67 artifacts; each tablet face linked). Inscription images © EFA and the 3D models © INSCRIBE/Università di Bologna are NOT included — links only. rete build --pyramid-algo types --text-index --card." },
...
SELECT ?year (COUNT(*) AS ?territories) WHERE {
  ?t ex:year ?year
    ...
    "lineara": [
      ...
      {"family": "Select", "label": "Tablets you can turn in 3D (INSCRIBE)", "view": "table", "tip": "67 of these artifacts have been scanned in high resolution by the ERC INSCRIBE project (Silvia Ferrara, University of Bologna) and published as interactive 3DHOP viewers. ?model is a clickable link to the rotatable, measurable 3D model — HT 29, the Khania (KH) and Phaistos (PH) tablets, plus nodules and roundels — shown beside the document's transliteration. The scans are © INSCRIBE / Università di Bologna (acknowledge the project); the graph stores a link, not the model data.", "q": "PREFIX la: <https://lineara.xyz/prop/>\nPREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>\nSELECT ?inscription ?model ?transliteration WHERE {\n  ?i la:model3d ?model ; rdfs:label ?inscription .\n  OPTIONAL { ?i la:transliteration ?transliteration }\n} ORDER BY ?inscription"},
