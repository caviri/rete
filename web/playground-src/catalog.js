...
window.RETE_PLAYGROUND_CATALOG = {
  ...
  datasets: [
    ...
    {"key": "arxiu", "kind": "remote-lazy", "url": "https://data.graphplaza.com/arxiu/arxiu.rete", "label": "arxiu.rete — Arxiu Històric de Tarragona: a complete Catalan archive, with scanned documents", "description": "The COMPLETE Arxiu Històric de Tarragona as it appears in the Generalitat de Catalunya's online archives (Arxius en Línia) — every one of its 7,401 archival units that has a digital image, across 63 fonds (the Govern Civil de Tarragona, the Yxart and Martí family archives, medieval charters, the historical land registry …). Each unit carries its holding archive, fonds, reference code, dates, description AND its scanned document. Non-reserved scans (3,101) are shown as a downsized WebP (~180 KB, ~15% of the ~1.5 MB original) mirrored to R2 so they render inline in query results; every unit also keeps arx:originalImage → the original full-res JPEG on Gencat's own server (the source of record). This is one complete archive of many — the same open API holds ~1.43M image-records across ~65 Catalan archives (~1.5 TB), and the harvester scales archive by archive. Modelled with schema.org archival vocabulary (schema:ArchiveComponent held by an ArchiveOrganization, part of a Collection/fonds) on w3id.org/rete. Descriptions are openly reusable (Llicència oberta d'ús d'informació / Llei 37/2007, Generalitat de Catalunya, with attribution); the reservat rights flag is respected — reserved units (~56% here) keep only the original link, never re-hosted."},
  ],
  ...
  datasetMeta: {
    ...
    "arxiu":                 { triples: "89 K", size: "1.6 MB", license: "Descriptions: Llicència oberta / CC0 (Llei 37/2007); images © Arxiu Històric de Tarragona", source: "https://arxiusenlinia.cultura.gencat.cat/", provenance: "Harvested 2026-07-13 — the COMPLETE Arxiu Històric de Tarragona (codiArxiu=330, 7,401 units with images, 63 fonds) from the open backend API (backend.arxiusenlinia.cultura.gencat.cat/unitat/search/advanced, mostrar=AMB_FITXER, sort=id for stable complete pagination). scripts/arxiu/harvest_and_mirror.py pages the archive + mirrors each non-reserved image as a downsized WebP (max 1400px, q80, ~15% of original, 3,101 of them) to R2 while keeping the original Gencat link; scripts/arxiu/build_arxiu_graph.py (accumulative over records_*.jsonl) → schema.org archival N-Triples → rete build --pyramid-algo types --text-index --card. One archive of ~65; full archive ~1.43M image-records (~1.5 TB)." },
  ...
  datasetExtra: {
    ...
    "arxiu":                 { icon: "🗃️", tags: ["archives","Tarragona","Catalonia","Generalitat","manuscripts","scanned documents","WebP","schema.org","open data","complete archive"] },
  ...
  examples: {
    ...
    "arxiu": [
      {"family": "Summary", "label": "📚 The fonds of the archive", "view": "table", "cols": {"fonds": "Fonds", "units": "Units"}, "tip": "The 63 fonds (record groups) that make up the Arxiu Històric de Tarragona — the Govern Civil, family archives, notarial and parish series — with how many digitised units each holds.\nEvery unit schema:isPartOf a fonds (a schema:Collection).", "q": "PREFIX schema: <http://schema.org/>\nSELECT ?fonds (COUNT(?u) AS ?units) WHERE {\n  ?u schema:isPartOf ?f . ?f schema:name ?fonds .\n} GROUP BY ?fonds ORDER BY DESC(?units)"},
