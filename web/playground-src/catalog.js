...
window.RETE_PLAYGROUND_CATALOG = {
  ...
  datasetMeta: {
    ...
    "arxiu":                 { triples: "2.2 M", size: "24.1 MB", license: "Descriptions: Llicència oberta / CC0 (Llei 37/2007); images © holding archives", source: "https://arxiusenlinia.cultura.gencat.cat/", provenance: "Harvested 2026-07-13, whole archive by whole archive, from the open backend API (backend.arxiusenlinia.cultura.gencat.cat/unitat/search/advanced, mostrar=AMB_FITXER, sort=id for stable complete pagination): Arxiu Històric de Tarragona (330, 7,401 units) + Arxiu Comarcal del Baix Llobregat (50, 53,644) + Arxiu Comarcal de l'Anoia (10, 137,054) = 198,099 units, 207 fonds, 8,634 WebP scans (non-reserved photos + PDF first-pages rendered via PyMuPDF). scripts/arxiu/harvest_and_mirror.py pages each archive + mirrors non-reserved images as downsized WebP (max 1400px, q80) to R2 while keeping the original Gencat link; scripts/arxiu/build_arxiu_graph.py (accumulative over records_*.jsonl, deduped) → schema.org archival N-Triples → rete build --pyramid-algo types --text-index --card. GEO-ENRICHED (scripts/arxiu/enrich_geo.py): 6,531 units carry a geo:asWKT municipal POINT (titles’ “Terme municipal de X” matched against 951 Catalan municipalities from Wikidata P625) and 5,340 a schema:contentLocation link to their PROVINCE’s geoadmin IRI — the cross-dataset join key. Full archive ~1.43M image-records across ~65 archives." },
