...
SELECT ?year (COUNT(*) AS ?territories) WHERE {
  ?t ex:year ?year
    ...
    "z-anatomy": [
      ...
      {"family": "Select", "label": "Diseases & phenotypes located in an organ (HPO + Disease Ontology)", "view": "table", "tip": "The cross-ontology join: each structure is lexically linked to Human Phenotype Ontology terms (anat:relatedPhenotype) and Human Disease Ontology terms (anat:relatedDisease) whose label names it. For the liver: 'polycystic liver disease', 'alcohol-associated liver disease', 'nutmeg liver' (diseases) and 'Abnormality of the liver', 'Hepatic agenesis' (phenotypes). Change \"Liver\" to \"Spleen\", \"Kidney\", \"Cerebellum\", \"Femur\".", "q": "PREFIX anat: <https://w3id.org/rete/anatomy#>\nPREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>\nSELECT ?kind ?term WHERE {\n  ?organ rdfs:label \"Liver\"@en .\n  { ?organ anat:relatedDisease ?t . BIND(\"disease (DOID)\" AS ?kind) }\n  UNION\n  { ?organ anat:relatedPhenotype ?t . BIND(\"phenotype (HP)\" AS ?kind) }\n  ?t rdfs:label ?term . FILTER(langMatches(lang(?term), \"en\"))\n} ORDER BY ?kind LIMIT 20"},
