...
window.RETE_PLAYGROUND_CATALOG = {
  ...
  examples: {
    "dblp": [
      ...
      {"family": "Aggregate", "label": "Publications by type", "view": "table", "cols": {"label": "Type", "n": "Count"},
       "tip": "The shape of the bibliography by DBLP record kind, labelled in English from the embedded dblp.ttl ontology: journal Articles (4.36M), Conference papers (3.91M), Book chapters, Books, Theses… plus the 5.79M Person creators and 4.14M person home-pages.\nThis scans the corpus (there is no Schema pyramid on this file yet) — the selective queries above are the cheap ones over HTTP range.",
       "q": "PREFIX dblp: <https://w3id.org/rete/dblp#>\nPREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>\nSELECT ?label (COUNT(?p) AS ?n) WHERE {\n  ?p a ?type .\n  ?type rdfs:label ?label .\n  FILTER(STRSTARTS(STR(?type), STR(dblp:)))\n} GROUP BY ?type ?label ORDER BY DESC(?n)"},
