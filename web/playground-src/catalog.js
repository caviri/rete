...
window.RETE_PLAYGROUND_CATALOG = {
  ...
  datasetMeta: {
    ...
    "arxiu":                 { triples: "688 K", size: "7.2 MB", license: "Descriptions: Llicència oberta / CC0 (Llei 37/2007); images © holding archives", source: "https://arxiusenlinia.cultura.gencat.cat/", provenance: "Harvested 2026-07-13, whole archive by whole archive, from the open backend API (backend.arxiusenlinia.cultura.gencat.cat/unitat/search/advanced, mostrar=AMB_FITXER, sort=id for stable complete pagination): Arxiu Històric de Tarragona (330, 7,401 units) + Arxiu Comarcal del Baix Llobregat (50, 53,644 units), Anoia (10) in progress. scripts/arxiu/harvest_and_mirror.py pages each archive + mirrors non-reserved images as downsized WebP (max 1400px, q80, ~15% of original) to R2 while keeping the original Gencat link; scripts/arxiu/build_arxiu_graph.py (accumulative over records_*.jsonl, deduped) → schema.org archival N-Triples → rete build --pyramid-algo types --text-index --card. Full archive ~1.43M image-records (~1.5 TB) across ~65 archives." },
