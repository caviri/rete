...
window.RETE_PLAYGROUND_CATALOG = {
  ...
  datasetMeta: {
    ...
    "bioexplora":            { triples: "7.19 M",  size: "43 MB",   license: "CC BY 4.0 (specimens/images; audio CC BY-NC-ND)", source: "https://www.bioexplora.cat", provenance: "The 6 MCNB Darwin Core archives from GBIF/ipt.gbif.es (207,163 specimens, real dwc: term IRIs) + 13,543 IIIF images (iiif/portraitMedia, CORS-open, render inline) + 43,826 GeoSPARQL points + 672 type specimens; plus 667 Sketchfab 3D links (account laboratorinatura) and 173 Xeno-canto audio recordings (Eloisa Matheu, CC BY-NC-ND, not MCNB). Built KEYLESS by scripts/bioexplora_to_nt.py + rete build --pyramid-algo types --text-index --card. Remote-lazy. CC BY 4.0 (MCNB) except the audio." },
    "smithsonian3d":         { triples: 15239,     size: "374 KB",  license: "CC0 1.0 (public domain)", source: "https://3d.si.edu", provenance: "The Smithsonian Open Access 3D collection, harvested KEYLESS from the public smithsonian-open-access S3 bucket (3d/ prefix; each model a Voyager scene.svx.json -> Medium-quality Draco .glb + title + EDAN record id) by scripts/smithsonian3d_to_nt.py. 2,199 models, each with prop:mesh (the streamable .glb, rendered inline by the 3D cell), prop:unit (museum), prop:catalogNumber and prop:record (link to si.edu). rete build --pyramid-algo types --text-index --card. Remote-lazy from the bucket; lossless Parquet/DuckDB/SQLite companions for the Explore tab via scripts/rdf_to_entity_tables.py --nt --type-predicate rdf:type. CC0 - credit the Smithsonian." },
...
SELECT ?year (COUNT(*) AS ?territories) WHERE {
  ?t ex:year ?year
    ...
    "bioexplora": [
      ...
      {"family": "Select", "label": "3D skull scans (Atles osteologic)", "view": "table", "tip": "667 skull and bone 3D scans from the museum's Atles osteologic, on Sketchfab (account laboratorinatura). The ?model3d column is a 🧊 3D ↗ launch to the interactive viewer — turn the actual cranium of a hyena, fox or bird. CC BY.", "q": "PREFIX class: <https://bioexplora.cat/class/>\nPREFIX p: <https://bioexplora.cat/prop/>\nPREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>\nSELECT ?model ?model3d WHERE {\n  ?m a class:Model3D ; rdfs:label ?model ; p:sketchfab ?model3d .\n} LIMIT 40"},
