...
SELECT ?sub ?name WHERE {
  ...
  OPTIONAL { ?sub rdfs:label ?name . FILTER(LANG(?name) = "en") }
      ...
      },
      {
        ...
        view: "graph",
        tip: "A whole-gigabyte graph queried live in the browser: this occupation intersection - people who are both wdt:P106 physicist (Q169470) and philosopher (Q4964182) - faults in only ~40 MB of the 1.04 GB file, while the rest never leaves the server. Two columns pair each entity's Q-id with its English name (Ilya Prigogine and other scientist-philosophers).",
        q: `PREFIX wdt: <http://www.wikidata.org/prop/direct/>
PREFIX wd: <http://www.wikidata.org/entity/>
PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>
SELECT ?p ?who WHERE {
