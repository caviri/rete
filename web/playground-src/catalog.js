...
window.RETE_PLAYGROUND_CATALOG = {
  ...
  datasetMeta: {
    ...
    "geoadmin":              { triples: "376.6 K", size: "378 MB",  license: "CC-BY 4.0 (geoBoundaries) + CC0 (Natural Earth places)", source: "https://www.geoboundaries.org", provenance: "geoBoundaries (OSM-derived, CC-BY 4.0) global admin boundaries CGAZ + Natural Earth places, converted to GeoSPARQL by scripts/geoboundaries_to_nt.py: 218 Country polygons (ADM0), 3,224 Region polygons (ADM1), 49,312 District polygons (ADM2), 1,251 populated-Place points. GeoJSON → WKT by hand (no GDAL); each entity with p:name, p:iso/p:country (alpha-2 mapped from geoBoundaries' alpha-3 via datasets/country-codes), p:adminLevel, p:partOf and geo:asWKT. 1.1 GB of N-Triples -> a 378 MB .rete (~3x: JSON overhead dropped + zstd on the WKT dictionary). Built as a FEDERATION partner: join another dataset's country code (e.g. dwc:countryCode) to attach the boundary geometry. rete build --pyramid-algo types --card (no text-index — WKT text isn't worth indexing)." },
...
SELECT ?year (COUNT(*) AS ?territories) WHERE {
  ?t ex:year ?year
    ...
    "geoadmin": [
      {"family": "Aggregate", "label": "Countries of the world", "view": "map", "cols": {"country": "Country", "boundary": "Boundary"}, "tip": "All 218 country polygons (geoBoundaries ADM0, OSM-derived, detailed). Output → Map draws the world; in Table view each ?boundary cell is a mini-map you can click to open full-screen.", "q": "PREFIX g: <https://geoadmin.rete/prop/>\nPREFIX geo: <http://www.opengis.net/ont/geosparql#>\nPREFIX class: <https://geoadmin.rete/class/>\nSELECT ?country ?boundary WHERE {\n  ?c a class:Country ; g:name ?country ; geo:asWKT ?boundary .\n} LIMIT 200"},
      {"family": "Select", "label": "One country's boundary", "view": "map", "cols": {"country": "Country", "boundary": "Boundary"}, "tip": "A single country polygon by ISO code. Change \"ES\" to FR, MA, IT, KE, PH… Click the map cell (Table view) for a full zoomable map.", "q": "PREFIX g: <https://geoadmin.rete/prop/>\nPREFIX geo: <http://www.opengis.net/ont/geosparql#>\nSELECT ?country ?boundary WHERE {\n  ?c g:iso \"ES\" ; g:name ?country ; geo:asWKT ?boundary .\n}"},
      {"family": "Aggregate", "label": "Regions / states (admin-1)", "view": "map", "cols": {"region": "Region", "boundary": "Boundary"}, "tip": "3,224 first-level admin areas (geoBoundaries ADM1) as detailed OSM polygons — a second admin level on the same GeoSPARQL model.", "q": "PREFIX g: <https://geoadmin.rete/prop/>\nPREFIX geo: <http://www.opengis.net/ont/geosparql#>\nPREFIX class: <https://geoadmin.rete/class/>\nSELECT ?region ?boundary WHERE {\n  ?s a class:Region ; g:name ?region ; geo:asWKT ?boundary .\n} LIMIT 400"},
      {"family": "Select", "label": "Districts of a country (admin-2)", "view": "map", "cols": {"district": "District", "boundary": "Boundary"}, "tip": "49,312 second-level admin areas (geoBoundaries ADM2). Here every district of Spain (ES) — change the code to FR, MA, IT… Detailed OSM boundaries; click a cell for the full zoomable map.", "q": "PREFIX g: <https://geoadmin.rete/prop/>\nPREFIX geo: <http://www.opengis.net/ont/geosparql#>\nPREFIX class: <https://geoadmin.rete/class/>\nSELECT ?district ?boundary WHERE {\n  ?d a class:District ; g:country \"ES\" ; g:name ?district ; geo:asWKT ?boundary .\n} LIMIT 400"},
