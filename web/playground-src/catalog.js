...
window.RETE_PLAYGROUND_CATALOG = {
  ...
  datasetMeta: {
    ...
    "geoadmin":              { triples: "369.3 K", size: "15 MB",   license: "CC-BY 4.0 (geoBoundaries) + CC0 (Natural Earth places)", source: "https://www.geoboundaries.org", provenance: "geoBoundaries (OSM-derived, CC-BY 4.0) global admin boundaries CGAZ + Natural Earth places, converted to GeoSPARQL by scripts/geoboundaries_to_nt.py: 213 Country polygons (ADM0), 3,133 Region polygons (ADM1), 48,362 District polygons (ADM2), 1,251 populated-Place points. GeoJSON → WKT by hand (no GDAL); each entity with p:name, p:iso/p:country (alpha-2 mapped from geoBoundaries' alpha-3 via datasets/country-codes), p:adminLevel, p:partOf and geo:asWKT. The detailed OSM boundaries (Spain alone was ~1 MB of WKT, the biggest ~21 MB) are SIMPLIFIED with Douglas–Peucker at ~1 km — visually identical on a map but ~10-40x fewer points: 1.1 GB of raw N-Triples → 81 MB simplified → a 15 MB .rete, and each cell now parses instantly. Built as a FEDERATION partner: join another dataset's country code (e.g. dwc:countryCode) to attach the boundary geometry. rete build --pyramid-algo types --card." },
...
SELECT ?year (COUNT(*) AS ?territories) WHERE {
  ?t ex:year ?year
    ...
    "geoadmin": [
      {"family": "Aggregate", "label": "Countries of the world", "view": "map", "cols": {"country": "Country", "boundary": "Boundary"}, "tip": "All 213 country polygons (geoBoundaries ADM0, OSM-derived, detailed). Output → Map draws the world; in Table view each ?boundary cell is a mini-map you can click to open full-screen.", "q": "PREFIX g: <https://geoadmin.rete/prop/>\nPREFIX geo: <http://www.opengis.net/ont/geosparql#>\nPREFIX class: <https://geoadmin.rete/class/>\nSELECT ?country ?boundary WHERE {\n  ?c a class:Country ; g:name ?country ; geo:asWKT ?boundary .\n} LIMIT 200"},
      ...
      {"family": "Aggregate", "label": "Regions / states (admin-1)", "view": "map", "cols": {"region": "Region", "boundary": "Boundary"}, "tip": "3,133 first-level admin areas (geoBoundaries ADM1) as detailed OSM polygons — a second admin level on the same GeoSPARQL model.", "q": "PREFIX g: <https://geoadmin.rete/prop/>\nPREFIX geo: <http://www.opengis.net/ont/geosparql#>\nPREFIX class: <https://geoadmin.rete/class/>\nSELECT ?region ?boundary WHERE {\n  ?s a class:Region ; g:name ?region ; geo:asWKT ?boundary .\n} LIMIT 400"},
