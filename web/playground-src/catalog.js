...
window.RETE_PLAYGROUND_CATALOG = {
  ...
  examples: {
    ...
    "dance": [
      ...
      {"family": "Reasoning", "label": "🧠 Every turn in the corpus", "view": "table", "cols": {"performance": "Performance", "start": "Start (s)", "move": "Move"}, "tip": "Turn on 🧠 Reason for this one. Segments are typed with the LEAF move (RightTurn / LeftTurn), never dance:Turn directly — but the OWL 2 QL move taxonomy has RightTurn ⊑ Turn ⊑ Move, so the reasoner returns all 521 turns anyway.\nSwap dance:Turn for dance:Figure (1,153) or dance:Move (3,968) to widen the net.", "q": "PREFIX dance: <https://w3id.org/rete/dance#>\nPREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>\nSELECT ?performance ?start ?move WHERE {\n  ?seg a dance:Turn ; dance:startTime ?start ; rdfs:label ?move .\n  ?perf dance:hasSegment ?seg ; rdfs:label ?performance .\n} ORDER BY ?performance ?start LIMIT 50"},
