...
window.RETE_PLAYGROUND_CATALOG = {
  ...
  datasets: [
    ...
    {"key": "databnf-persons", "kind": "remote-lazy", "url": "https://katospiegel-rete.hf.space/data/playground/databnf/databnf-persons.rete?token=sfdbgf1094by21hd128ru39802", "typePredicate": "<http://www.w3.org/1999/02/22-rdf-syntax-ns#type>", "label": "data.bnf.fr (authors & persons) - 4.9M author/person authorities, 108M triples, with VIAF/ISNI/IdRef links; the federation hub (CC0, remote-lazy)", "description": "The PERSONS shard of data.bnf.fr - the BnF's linked open data (CC0). 4.9M author/person authorities (foaf:Person) with names, dates and 8.8M owl:sameAs alignments to VIAF / ISNI / IdRef - the authority layer that federates with the authors in MMM, Jonas and Biblissima. data.bnf.fr is huge (716M triples), so it is split into per-perimeter shards (persons, works, rameau, contributions, editions...) all hosted under playground/databnf/; query one and use '+ Add source' to federate the rest (union + dedup). 108.8M triples, queried lazily over HTTP range. Entity IRIs stay as data.bnf.fr/ark:/* so nodes round-trip to the live BnF. CC0 1.0 (BnF)."},
    {"key": "databnf-works", "kind": "remote-lazy", "url": "https://katospiegel-rete.hf.space/data/playground/databnf/databnf-works.rete?token=sfdbgf1094by21hd128ru39802", "typePredicate": "<http://www.w3.org/1999/02/22-rdf-syntax-ns#type>", "label": "data.bnf.fr (works) - 2.6M FRBR works, 64M triples: titles, creators, RAMEAU subjects (CC0, remote-lazy)", "description": "The WORKS shard of data.bnf.fr (CC0): 2.6M works (FRBR/RDA) with multilingual titles (dcterms:title), their creator (dcterms:creator -> a data.bnf.fr author ARK; federate with databnf-persons for names), subjects (RAMEAU), forms and dates, plus the link to their manifestations. Part of the sharded data.bnf.fr (716M); use '+ Add source' to federate. 63.7M triples, remote-lazy. CC0 1.0 (BnF)."},
    {"key": "databnf-rameau", "kind": "remote-lazy", "url": "https://katospiegel-rete.hf.space/data/playground/databnf/databnf-rameau.rete?token=sfdbgf1094by21hd128ru39802", "label": "data.bnf.fr (RAMEAU subjects) - the French national subject thesaurus, 37M triples (CC0, remote-lazy)", "description": "The RAMEAU shard of data.bnf.fr (CC0): the French national subject-heading thesaurus - millions of skos:Concept subjects with prefLabels, broader/narrower hierarchy and the documents indexed under each. Part of the sharded data.bnf.fr (716M); federate via '+ Add source'. 36.6M triples, remote-lazy. CC0 1.0 (BnF)."},
    {"key": "databnf-contributions", "kind": "remote-lazy", "url": "https://katospiegel-rete.hf.space/data/playground/databnf/databnf-contributions.rete?token=sfdbgf1094by21hd128ru39802", "label": "data.bnf.fr (contributions) - role-typed agent->work/edition links, 68M triples (CC0, remote-lazy)", "description": "The CONTRIBUTIONS shard of data.bnf.fr (CC0): the role-typed links between agents and the works/editions they contributed to (author, illustrator, translator, editor...). Part of the sharded data.bnf.fr (716M); federate via '+ Add source'. 67.7M triples, remote-lazy. CC0 1.0 (BnF)."},
    {"key": "databnf-misc", "kind": "remote-lazy", "url": "https://katospiegel-rete.hf.space/data/playground/databnf/databnf-misc.rete?token=sfdbgf1094by21hd128ru39802", "label": "data.bnf.fr (places & periodicals) - places (GeoNames), periodicals, orgs & vocab, 30M triples (CC0, remote-lazy)", "description": "The MISC shard of data.bnf.fr (CC0): places (with GeoNames links), periodicals, organisations, musical works & genres, performances, country/role codes and the bnf-onto vocabulary. Part of the sharded data.bnf.fr (716M); federate via '+ Add source'. 29.8M triples, remote-lazy. CC0 1.0 (BnF)."},
    {"key": "databnf-editions", "kind": "remote-lazy", "url": "https://katospiegel-rete.hf.space/data/playground/databnf/databnf-editions-00.rete?token=sfdbgf1094by21hd128ru39802", "typePredicate": "<http://www.w3.org/1999/02/22-rdf-syntax-ns#type>", "label": "data.bnf.fr (editions, shard 1/5) - the printed-book catalogue, 100M triples per shard (CC0, remote-lazy)", "description": "The EDITIONS perimeter of data.bnf.fr (CC0) - the modern printed-book catalogue (manifestations/expressions). 410M triples split into 5 shards databnf-editions-00..04 (all hosted under playground/databnf/); this entry is the first. It is the bulk of data.bnf.fr (57%); the authority shards (databnf-persons / works / rameau) are the more knowledge-graph-useful layer. 100M triples per shard, remote-lazy. CC0 1.0 (BnF)."},
    {"key": "biblissima", "kind": "remote-lazy", "url": "https://katospiegel-rete.hf.space/data/playground/biblissima/biblissima-00.rete?token=sfdbgf1094by21hd128ru39802", "label": "Biblissima+ (shard 1/3) - medieval written-heritage Wikibase, 90M of 254M triples (CC-BY, remote-lazy)", "description": "Biblissima+, the French aggregator of medieval & early-modern written heritage, as RDF - shard 1 of 3. The whole graph is ~254M triples over 867,838 Wikibase entities (manuscripts, works, persons, places, editions, iconography), harvested from its open per-entity RDF export (Special:EntityData; Biblissima has no bulk dump and its SPARQL is gated). The authority / manuscript hub of this whole constellation - it overlaps the BnF, MMM and Jonas on persons & places. Wikibase model: entities at data.biblissima.fr/entity/Q*, statements via prop/direct/P*, multilingual rdfs:label / skos:prefLabel. Split into 3 --no-pyramid shards (biblissima-00..02, all hosted under playground/biblissima/); query one and use '+ Add source' to federate the rest. This shard: 90M triples, remote-lazy. CC-BY 4.0 (Biblissima+, Campus Condorcet)."},
    {"key": "biblissima-1", "kind": "remote-lazy", "url": "https://katospiegel-rete.hf.space/data/playground/biblissima/biblissima-01.rete?token=sfdbgf1094by21hd128ru39802", "label": "Biblissima+ (shard 2/3) - medieval written-heritage Wikibase, 90M triples (CC-BY, remote-lazy)", "description": "Shard 2 of 3 of the Biblissima+ Wikibase graph (254M triples total; see the biblissima entry). 90M triples, remote-lazy; federate with biblissima + biblissima-2 via '+ Add source'. CC-BY 4.0 (Biblissima+)."},
    {"key": "biblissima-2", "kind": "remote-lazy", "url": "https://katospiegel-rete.hf.space/data/playground/biblissima/biblissima-02.rete?token=sfdbgf1094by21hd128ru39802", "label": "Biblissima+ (shard 3/3) - medieval written-heritage Wikibase, 74.2M triples (CC-BY, remote-lazy)", "description": "Shard 3 of 3 of the Biblissima+ Wikibase graph (254M triples total; see the biblissima entry). 74.2M triples, remote-lazy; federate with biblissima + biblissima-1 via '+ Add source'. CC-BY 4.0 (Biblissima+)."},
  ...
  datasetMeta: {
    ...
    "databnf-persons":       { triples: "108.8 M", size: "1.5 GB",  license: "CC0 1.0",              source: "https://data.bnf.fr/",                                     provenance: "data.bnf.fr official RDF dump (data.gouv.fr, CC0) - the Auteurs + Personnes elementaires + Organisations perimeters - streamed from the .tar.gz N-Triples into rete build --no-pyramid (one of 10 federated shards of the 716M-triple data.bnf.fr; see scripts/build_databnf_shards.sh)." },
    "databnf-works":         { triples: "63.7 M",  size: "596 MB",  license: "CC0 1.0",              source: "https://data.bnf.fr/",                                     provenance: "data.bnf.fr official dump (CC0): Oeuvres + Oeuvres calculees + Oeuvres musicales + Etudes perimeters, rete build --no-pyramid shard." },
    "databnf-rameau":        { triples: "36.6 M",  size: "221 MB",  license: "CC0 1.0",              source: "https://data.bnf.fr/",                                     provenance: "data.bnf.fr official dump (CC0): Rameau (+ hors-indexation, documents indexes) perimeters, rete build --no-pyramid shard." },
    "databnf-contributions": { triples: "67.7 M",  size: "674 MB",  license: "CC0 1.0",              source: "https://data.bnf.fr/",                                     provenance: "data.bnf.fr official dump (CC0): Contributions perimeter, rete build --no-pyramid shard." },
    "databnf-misc":          { triples: "29.8 M",  size: "443 MB",  license: "CC0 1.0",              source: "https://data.bnf.fr/",                                     provenance: "data.bnf.fr official dump (CC0): places, periodicals, organisations, musical genres, performances, IMSLP alignments, codes and bnf-onto vocab, rete build --no-pyramid shard." },
    "databnf-editions":      { triples: "410 M (5 shards)", size: "3.9 GB", license: "CC0 1.0",      source: "https://data.bnf.fr/",                                     provenance: "data.bnf.fr official dump (CC0): the Editions perimeter (410M triples) split by 100M-line N-Triples chunks into 5 --no-pyramid shards (databnf-editions-00..04); see scripts/build_databnf_editions.sh." },
    "biblissima":            { triples: "254 M (3 shards)", size: "403 MB", license: "CC-BY 4.0",     source: "https://data.biblissima.fr/",                              provenance: "Biblissima+ Wikibase (867,838 entities) harvested entity-by-entity via the open Special:EntityData .nt export (scripts/harvest_biblissima.py; no bulk dump, SPARQL gated), then split into 90M-line N-Triples chunks and built as 3 --no-pyramid shards biblissima-00..02 (scripts/build_biblissima_shards.sh) - the monolithic 254M build OOMs a 62 GB host." },
    "biblissima-1":          { triples: "90 M",    size: "140 MB",  license: "CC-BY 4.0",            source: "https://data.biblissima.fr/",                              provenance: "Shard 2/3 of the Biblissima+ Wikibase graph; see biblissima." },
    "biblissima-2":          { triples: "74.2 M",  size: "111 MB",  license: "CC-BY 4.0",            source: "https://data.biblissima.fr/",                              provenance: "Shard 3/3 of the Biblissima+ Wikibase graph; see biblissima." },
  ...
  datasetExtra: {
    ...
    "databnf-persons":       { icon: "🇫🇷", tags: ["BnF", "authors", "VIAF", "authorities", "federation"] },
    "databnf-works":         { icon: "🇫🇷", tags: ["BnF", "works", "FRBR", "titles"] },
    "databnf-rameau":        { icon: "🏷️", tags: ["BnF", "RAMEAU", "subjects", "SKOS", "thesaurus"] },
    "databnf-contributions": { icon: "🇫🇷", tags: ["BnF", "contributions", "roles"] },
    "databnf-misc":          { icon: "🇫🇷", tags: ["BnF", "places", "periodicals", "GeoNames"] },
    "databnf-editions":      { icon: "📚", tags: ["BnF", "editions", "manifestations", "catalogue"] },
    "biblissima":            { icon: "🗝️", tags: ["Biblissima", "manuscripts", "Wikibase", "heritage", "federation"] },
    "biblissima-1":          { icon: "🗝️", tags: ["Biblissima", "Wikibase", "shard"] },
    "biblissima-2":          { icon: "🗝️", tags: ["Biblissima", "Wikibase", "shard"] },
...
SELECT ?year (COUNT(*) AS ?territories) WHERE {
  ?t ex:year ?year
    ...
    "databnf-persons": [
      {"family": "Select", "label": "Authors and their VIAF / ISNI identity", "view": "table", "tip": "data.bnf.fr's authority layer: each author (foaf:Person) carries a name and owl:sameAs alignments to VIAF, ISNI and IdRef - the SAME VIAF ids that MMM, Jonas and Biblissima link to, so this shard is the federation hub for 'who is this author' across every manuscript dataset (8.8M sameAs links here). The leading space in some names is BnF's sort form.", "q": "PREFIX foaf: <http://xmlns.com/foaf/0.1/>\nPREFIX owl: <http://www.w3.org/2002/07/owl#>\nSELECT ?name ?authority WHERE {\n  ?p a foaf:Person ; foaf:name ?name ; owl:sameAs ?authority .\n  FILTER(CONTAINS(STR(?authority), \"viaf\"))\n} LIMIT 100"},
      {"family": "Summary", "label": "What's in the persons shard", "view": "table", "tip": "The shape of the authority shard: foaf:Person with names, family/given names, dates (bnf-onto), the FRBNF id and millions of owl:sameAs to external authorities. ~4.9M person authorities over 108.8M triples; add the other databnf shards with '+ Add source' to federate.", "q": "SELECT ?p (COUNT(*) AS ?n) WHERE { ?s ?p ?o } GROUP BY ?p ORDER BY DESC(?n) LIMIT 20"}
    ],
    "databnf-works": [
      {"family": "Select", "label": "Works with their titles and creators", "view": "table", "tip": "2.6M works (FRBR/RDA): each with a multilingual title (dcterms:title) and its creator as a data.bnf.fr author ARK (dcterms:creator). Add databnf-persons as a source to resolve that ARK to a name. The @fr / @la / @cs tag on each title is its language.", "q": "PREFIX dct: <http://purl.org/dc/terms/>\nSELECT ?title ?creator WHERE {\n  ?w dct:title ?title ; dct:creator ?creator .\n} LIMIT 100"},
      {"family": "Summary", "label": "What's in the works shard", "view": "table", "tip": "The shape of the works shard: workManifested (-> editions), dcterms:subject (RAMEAU), formOfWork, dcterms:creator and owl:sameAs. ~2.6M works over 63.7M triples.", "q": "SELECT ?p (COUNT(*) AS ?n) WHERE { ?s ?p ?o } GROUP BY ?p ORDER BY DESC(?n) LIMIT 20"}
    ],
    "databnf-rameau": [
      ...
      {"family": "Summary", "label": "What's in the RAMEAU shard", "view": "table", "tip": "Predicate totals over the RAMEAU subject thesaurus (36.6M triples): prefLabels, the broader/narrower SKOS hierarchy, and the documents indexed under each heading.", "q": "SELECT ?p (COUNT(*) AS ?n) WHERE { ?s ?p ?o } GROUP BY ?p ORDER BY DESC(?n) LIMIT 20"}
    ],
    "databnf-contributions": [
      {"family": "Summary", "label": "What's in the contributions shard", "view": "table", "tip": "The role-typed links between agents and the works/editions they contributed to (author, illustrator, translator, editor...). 67.7M triples - federate with databnf-persons + databnf-works (via '+ Add source') to follow a contribution to its agent and its work.", "q": "SELECT ?p (COUNT(*) AS ?n) WHERE { ?s ?p ?o } GROUP BY ?p ORDER BY DESC(?n) LIMIT 20"}
    ],
    "databnf-misc": [
      {"family": "Summary", "label": "What's in the misc shard", "view": "table", "tip": "Predicate totals over the misc shard (29.8M triples): places (with GeoNames owl:sameAs), periodicals, organisations, musical works & genres, performances, country/role codes and the bnf-onto vocabulary.", "q": "SELECT ?p (COUNT(*) AS ?n) WHERE { ?s ?p ?o } GROUP BY ?p ORDER BY DESC(?n) LIMIT 20"}
    ],
    "databnf-editions": [
      {"family": "Summary", "label": "What's in the editions (shard 1 of 5)", "view": "table", "tip": "The modern printed-book catalogue (manifestations / expressions) - the bulk of data.bnf.fr (410M triples across 5 shards). This is shard 1/5 (100M triples); add databnf-editions-01..04 as sources to span the whole perimeter. Predicate totals over this slice.", "q": "SELECT ?p (COUNT(*) AS ?n) WHERE { ?s ?p ?o } GROUP BY ?p ORDER BY DESC(?n) LIMIT 20"}
    ],
    "biblissima": [
      {"family": "Summary", "label": "What's in this shard (predicate totals)", "view": "table", "tip": "The shape of the Biblissima+ Wikibase: rdf:type, wikibase:rank, multilingual labels (rdfs:label / skos:prefLabel / schema:name) and the prop/direct/P* statements. This is 1 of 3 shards (90M of the 254M-triple graph); add biblissima-1 + biblissima-2 with '+ Add source' to span the whole.", "q": "SELECT ?p (COUNT(*) AS ?n) WHERE { ?s ?p ?o } GROUP BY ?p ORDER BY DESC(?n) LIMIT 25"},
      ...
      {"family": "Aggregate", "label": "Entities grouped by type (instance of)", "view": "table", "tip": "Group entities by their type (prop/direct/P2 = 'instance of' in the Biblissima Wikibase), resolved to a French label - the composition of the graph (manuscripts, works, persons, places...). Within-shard counts; federate the other two shards for the full totals.", "q": "PREFIX bd: <https://data.biblissima.fr/prop/direct/>\nPREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>\nSELECT ?typeLabel (COUNT(?s) AS ?n) WHERE {\n  ?s bd:P2 ?type . ?type rdfs:label ?typeLabel . FILTER(LANG(?typeLabel) = \"fr\")\n} GROUP BY ?typeLabel ORDER BY DESC(?n) LIMIT 20"}
    ],
    "albala": [
