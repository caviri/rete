...
SELECT ?year (COUNT(*) AS ?territories) WHERE {
  ?t ex:year ?year
    ...
    "biblissima": [
      {"family": "Select", "label": "Manuscripts you can view online (IIIF)", "view": "table", "tip": "A URL-ID column on real heritage data: prop/direct/P196 links a manuscript to its IIIF manifest - the digitized object at its holding library (Cambridge cudl, e-codices, the Vatican's digi.vatlib.it, Gallica...). It renders as a clickable link straight to the manuscript online. ?manuscript is the Biblissima entity (also a link), ?label its shelfmark. Set a column's type dropdown to Link if it doesn't auto-detect.", "q": "PREFIX bd: <https://data.biblissima.fr/prop/direct/>\nPREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>\nSELECT ?manuscript ?label ?iiif WHERE {\n  ?manuscript bd:P196 ?iiif ; rdfs:label ?label .\n  FILTER(LANG(?label) = \"en\")\n} LIMIT 100"},
