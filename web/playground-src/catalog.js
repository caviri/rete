...
window.RETE_PLAYGROUND_CATALOG = {
  ...
  datasets: [
    ...
    {"key": "geoadmin", "kind": "remote-lazy", "url": "https://katospiegel-rete.hf.space/data/playground/geoadmin.rete?token=sfdbgf1094by21hd128ru39802", "label": "geoadmin.rete - world admin boundaries + places (Natural Earth, GeoSPARQL, remote, lazy)", "description": "World administrative boundaries and places with GeoSPARQL geometries, built for FEDERATED geospatial joins. 175 COUNTRIES and 294 STATES/provinces as POLYGONS, plus 1,251 populated PLACES as POINTS — different admin levels, points and polygons. Each entity carries a name, an ISO 3166-1 code (p:iso on a country, p:country on a state/place), an admin level (p:adminLevel) and its geometry (geo:asWKT). Add it as a federation source to ANY dataset and JOIN on the country code to pull in the boundary polygon — e.g. attach to every bioexplora specimen the polygon of the country it was collected in, then switch Output → Map. Public domain (Natural Earth, naturalearthdata.com); built by scripts/naturalearth_to_nt.py."},
  ...
  datasetMeta: {
    ...
    "geoadmin":              { triples: 11151,     size: "1.6 MB",  license: "CC0 / public domain (Natural Earth)", source: "https://www.naturalearthdata.com", provenance: "Natural Earth (public domain) admin boundaries + places, converted to GeoSPARQL by scripts/naturalearth_to_nt.py: 175 Country polygons (admin-0, 1:110m) with ISO 3166-1 codes, 294 State/province polygons (admin-1, 1:50m), 1,251 populated Place points (1:50m). GeoJSON → WKT, each entity with p:name, p:iso/p:country, p:adminLevel and geo:asWKT. Built as a FEDERATION partner: join another dataset's country code (e.g. dwc:countryCode) to attach the boundary geometry. rete build --pyramid-algo types --text-index --card. CC0 — Natural Earth." },
...
SELECT ?year (COUNT(*) AS ?territories) WHERE {
  ?t ex:year ?year
    ...
    "geoadmin": [
      {"family": "Aggregate", "label": "Countries of the world", "view": "map", "cols": {"country": "Country", "boundary": "Boundary"}, "tip": "All 175 country polygons (Natural Earth admin-0). Output → Map draws the world; in Table view each ?boundary cell is a mini-map you can click to open full-screen. Public domain.", "q": "PREFIX g: <https://natural-earth.rete/prop/>\nPREFIX geo: <http://www.opengis.net/ont/geosparql#>\nPREFIX class: <https://natural-earth.rete/class/>\nSELECT ?country ?boundary WHERE {\n  ?c a class:Country ; g:name ?country ; geo:asWKT ?boundary .\n} LIMIT 200"},
      {"family": "Select", "label": "One country's boundary", "view": "map", "cols": {"country": "Country", "boundary": "Boundary"}, "tip": "A single country polygon by ISO code. Change \"ES\" to FR, MA, IT, KE, PH… Click the map cell (Table view) for a full zoomable map.", "q": "PREFIX g: <https://natural-earth.rete/prop/>\nPREFIX geo: <http://www.opengis.net/ont/geosparql#>\nSELECT ?country ?boundary WHERE {\n  ?c g:iso \"ES\" ; g:name ?country ; geo:asWKT ?boundary .\n}"},
      {"family": "Aggregate", "label": "States & provinces (admin-1)", "view": "map", "cols": {"state": "State", "boundary": "Boundary"}, "tip": "294 first-level admin areas as polygons — a second admin level on the same GeoSPARQL model.", "q": "PREFIX g: <https://natural-earth.rete/prop/>\nPREFIX geo: <http://www.opengis.net/ont/geosparql#>\nPREFIX class: <https://natural-earth.rete/class/>\nSELECT ?state ?boundary WHERE {\n  ?s a class:State ; g:name ?state ; geo:asWKT ?boundary .\n} LIMIT 300"},
      {"family": "Aggregate", "label": "Populated places (points)", "view": "map", "cols": {"place": "Place", "point": "Location"}, "tip": "1,251 cities and towns as POINTS — points and polygons share one geometry model.", "q": "PREFIX g: <https://natural-earth.rete/prop/>\nPREFIX geo: <http://www.opengis.net/ont/geosparql#>\nPREFIX class: <https://natural-earth.rete/class/>\nSELECT ?place ?point WHERE {\n  ?p a class:Place ; g:name ?place ; geo:asWKT ?point .\n} LIMIT 1251"}
    ],
    "smithsonian3d": [
