...
window.RETE_PLAYGROUND_CATALOG = {
  ...
  datasets: [
    {"key": "farmacos-es", "kind": "remote-lazy", "url": "https://data.graphplaza.com/farmacos-es/farmacos-es.rete", "label": "farmacos-es.rete — todos los medicamentos autorizados en España (CIMA/AEMPS, remote, lazy)", "description": "Grafo de conocimiento del catálogo completo de medicamentos autorizados en España, construido a partir de la API REST pública del CIMA (AEMPS) y el Nomenclátor de Prescripción. 25,485 medicamentos (fmc:Medicamento ⊑ schema:Drug) con su condición de prescripción, fecha de autorización y flags (genérico/EFG, huérfano, biosimilar, triángulo negro, receta); 33,184 ingredientes reificados con dosis y unidad; 42,283 presentaciones (código nacional); principios activos, laboratorios (⊑ schema:MedicalOrganization), excipientes de declaración obligatoria, vías y formas farmacéuticas como skos:Concept. La clasificación ATC de la OMS está modelada como jerarquía SKOS completa (skos:broader + skos:exactMatch a BioPortal + rdfs:seeAlso a WHOCC), y las sustancias enlazan a SNOMED CT (http://snomed.info/id/) por la VTM del medicamento y por los códigos DCSA/DCP/DCPF de la extensión española de fármacos. Extracción médica estructurada: 48,997 interacciones farmacológicas a nivel ATC (fmc:Interaccion con efecto, recomendación y gravedad derivada — contraindicada / no recomendada / precaución / informativa) tomadas del Nomenclátor, y 761,945 secciones de prospecto y ficha técnica troceadas según la plantilla QRD europea (indicaciones, posología, efectos adversos, contraindicaciones, conservación, composición) con el texto completo indexado (TEXT_INDEX → CONTAINS) y las bandas de frecuencia de efectos adversos etiquetadas. 9.48 M triples, pirámide por tipos y Dataset Card embebida. scripts/farmacos-es/harvest.py (API oficial, sin scraping) + to_rdf.py → rete build --pyramid-algo types --text-index --card. Reutilización de información del sector público (Ley 37/2007); datos de la AEMPS. Remote-lazy en Cloudflare R2."},
