...
window.RETE_PLAYGROUND_CATALOG = {
  ...
  datasets: [
    ...
    {"key": "gbif-birds", "kind": "remote-lazy", "url": "https://data.graphplaza.com/gbif-birds/gbif-birds.rete", "typePredicate": "<http://www.w3.org/1999/02/22-rdf-syntax-ns#type>", "label": "GBIF Birds — Spain & Switzerland: 50M sightings with a taxonomic zoom + map (remote, lazy)", "description": "Fifty MILLION georeferenced bird (Aves) occurrences — Spain 47.9M, Switzerland 2.26M, 1,104 species — from the GBIF Open Data snapshot (2026-07-01), as one 300M-triple range-queryable file (1.2 GB, remote-lazy over HTTP range). This is the SEMANTIC-ZOOM dataset: every occurrence is rdf:type its species, and each species carries a full taxonomic subClassOf chain Species ⊑ Genus ⊑ Family ⊑ Order ⊑ Aves ⊑ Chordata ⊑ Animalia — so the schema pyramid rolls the 50M sightings up into a 6-LEVEL zoom you can read WITHOUT touching the triple index (open the Schema tab): level 3 = Orders (Passeriformes 29.7M, Charadriiformes 5.1M, Accipitriformes/raptors 2.9M, Anseriformes 2.2M …), level 4 = Families (Fringillidae, Muscicapidae, Accipitridae, Laridae …), level 5 = the 1,104 species. Ask a question about birds at the Order or Family level and you never descend to a single record — the abstraction is the point. Every occurrence also carries a GeoSPARQL POINT (map view), its country, year, basis-of-record (mostly citizen-science HUMAN_OBSERVATION — eBird & friends) and source dataset. IRIs round-trip to live GBIF: occurrence = gbif.org/occurrence/<id>, species owl:sameAs gbif.org/species/<key>. Built KEYLESS from the GBIF AWS Parquet snapshot via DuckDB (scripts/gbif_birds_extract.py + gbif_birds_to_nt.py) — no account, no scraping. CC BY-NC 4.0 (GBIF); cite GBIF.org. Selective queries (a species, a small area) and the Schema pyramid are cheap over HTTP range; a full 50M-row aggregate scans the file, so lean on the pyramid for the big picture."},
  ],
  ...
  datasetExtra: {
    ...
    "boe":                   { icon: "⚖️", tags: ["Spain", "law", "legislation", "ELI", "BOE", "consolidated", "citation-network", "derogation", "SKOS", "open-data", "government"] },
    ...
    "gbif-birds":            { icon: "🐦", tags: ["GBIF", "birds", "Aves", "biodiversity", "occurrences", "taxonomy", "semantic-zoom", "GeoSPARQL", "Spain", "Switzerland", "citizen-science"] },
  ...
  examples: {
    ...
    "gbif-birds": [
      {"family": "Path", "label": "★ Semantic zoom: the taxonomic ladder of a species", "view": "table", "cols": {"rank": "Rank", "name": "Taxon"},
       ...
       "q": "PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>\nPREFIX v: <https://rete.graphplaza.com/gbif/vocab#>\nSELECT ?rank ?name WHERE {\n  <https://rete.graphplaza.com/gbif/taxon/species/9515886> rdfs:subClassOf+ ?anc .\n  ?anc rdfs:label ?name ; v:rank ?rank .\n}"},
      {"family": "Select", "label": "🗺️ Map a species: Barn Swallow across Iberia & the Alps", "view": "map", "cols": {"wkt": "Location"},
       ...
       "q": "PREFIX geo: <http://www.opengis.net/ont/geosparql#>\nSELECT ?wkt WHERE {\n  ?o a <https://rete.graphplaza.com/gbif/taxon/species/9515886> ;\n     geo:asWKT ?wkt .\n} LIMIT 4000"},
      {"family": "Path", "label": "Every swallow species (family Hirundinidae)", "view": "table", "cols": {"name": "Species"},
       ...
       "q": "PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>\nPREFIX v: <https://rete.graphplaza.com/gbif/vocab#>\nSELECT ?name WHERE {\n  ?sp rdfs:subClassOf+ <https://rete.graphplaza.com/gbif/taxon/family/Hirundinidae> ;\n      rdfs:label ?name ; v:rank \"species\" .\n} ORDER BY ?name"},
      {"family": "Select", "label": "A species' records: House Sparrow, with country & year", "view": "table", "cols": {"occ": "Occurrence", "country": "Country", "year": "Year"},
       ...
       "q": "PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>\nPREFIX v: <https://rete.graphplaza.com/gbif/vocab#>\nPREFIX dwc: <http://rs.tdwg.org/dwc/terms/>\nSELECT ?occ ?country ?year WHERE {\n  ?occ a <https://rete.graphplaza.com/gbif/taxon/species/5231190> ;\n       v:inCountry ?c ; dwc:year ?year .\n  ?c rdfs:label ?country .\n} LIMIT 100"},
      {"family": "Select", "label": "🗺️ Map the raptors: where are the Accipitridae?", "view": "map", "cols": {"wkt": "Location"},
       ...
       "q": "PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>\nPREFIX geo: <http://www.opengis.net/ont/geosparql#>\nSELECT ?wkt WHERE {\n  ?sp rdfs:subClassOf+ <https://rete.graphplaza.com/gbif/taxon/family/Accipitridae> .\n  ?o a ?sp ; geo:asWKT ?wkt .\n} LIMIT 3000"},
      ...
      {"family": "Select", "label": "🗺️ Map the Bearded Vulture — Alps ↔ Pyrenees", "view": "map", "cols": {"wkt": "Location"},
       ...
       "q": "PREFIX geo: <http://www.opengis.net/ont/geosparql#>\nSELECT ?wkt WHERE {\n  ?o a <https://rete.graphplaza.com/gbif/taxon/species/2480649> ;\n     geo:asWKT ?wkt .\n} LIMIT 4000"},
      {"family": "Aggregate", "label": "📈 White Stork sightings by year", "view": "time", "cols": {"year": "Year", "n": "Sightings"},
       ...
       "q": "PREFIX v: <https://rete.graphplaza.com/gbif/vocab#>\nPREFIX dwc: <http://rs.tdwg.org/dwc/terms/>\nSELECT ?year (COUNT(?o) AS ?n) WHERE {\n  ?o a <https://rete.graphplaza.com/gbif/taxon/species/2481912> ; dwc:year ?year .\n}\nGROUP BY ?year ORDER BY ?year"},
      ...
      {"family": "Select", "label": "🗺️ Map a whole genus: the falcons (Falco)", "view": "map", "cols": {"wkt": "Location"},
       ...
       "q": "PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>\nPREFIX geo: <http://www.opengis.net/ont/geosparql#>\nSELECT ?wkt WHERE {\n  ?sp rdfs:subClassOf+ <https://rete.graphplaza.com/gbif/taxon/genus/Falco> .\n  ?o a ?sp ; geo:asWKT ?wkt .\n} LIMIT 3000"},
      ...
      {"family": "Select", "label": "Genera in a family: the ducks & geese (Anatidae)", "view": "table", "cols": {"genus": "Genus"},
       ...
       "q": "PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>\nPREFIX v: <https://rete.graphplaza.com/gbif/vocab#>\nSELECT ?genus WHERE {\n  ?g rdfs:subClassOf <https://rete.graphplaza.com/gbif/taxon/family/Anatidae> ;\n     v:rank \"genus\" ; rdfs:label ?genus .\n}\nORDER BY ?genus"},
      {"family": "Select", "label": "🗺️ Flamingos in Spain (Doñana, Ebro delta, Fuente de Piedra)", "view": "map", "cols": {"wkt": "Location"},
       ...
       "q": "PREFIX geo: <http://www.opengis.net/ont/geosparql#>\nPREFIX v: <https://rete.graphplaza.com/gbif/vocab#>\nSELECT ?wkt WHERE {\n  ?o a <https://rete.graphplaza.com/gbif/taxon/species/4352332> ;\n     geo:asWKT ?wkt ; v:inCountry <https://rete.graphplaza.com/gbif/country/ES> .\n} LIMIT 4000"},
