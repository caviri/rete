...
SELECT ?year (COUNT(*) AS ?territories) WHERE {
  ?t ex:year ?year
    ...
    "bioexplora": [
      ...
      {"family": "Select", "label": "Specimens you can SEE (photos)", "view": "table", "cols": {"species": "Species", "photo": "Photo"}, "tip": "~9,000 specimens are photographed. The ?photo column renders INLINE — molluscs, crustaceans, insects, fossils. Photos are mirrored to the bucket as fast WebP (p:preview, ~10-40 KB; the museum's source link is p:image). CC BY 4.0, credit the Museu de Ciencies Naturals de Barcelona.", "q": "PREFIX dwc: <http://rs.tdwg.org/dwc/terms/>\nPREFIX p: <https://bioexplora.cat/prop/>\nSELECT ?species ?photo WHERE {\n  ?s dwc:scientificName ?species ; p:preview ?photo .\n} LIMIT 40"},
      {"family": "Path", "label": "See it, hear it, place it — photo + sound + map", "view": "table", "cols": {"species": "Species", "photo": "Photo", "sound": "Call", "location": "Where found"}, "tip": "One row, four media: each PHOTOGRAPHED, GEOREFERENCED specimen joined to a Xeno-canto RECORDING of the SAME species (on dwc:scientificName). A bird like the Great Crested Grebe shows its specimen photo (WebP), its call (inline player) and where it was collected (click the map cell for a full zoomable map) — all together. Stay in Table view to see all four at once.", "q": "PREFIX dwc: <http://rs.tdwg.org/dwc/terms/>\nPREFIX p: <https://bioexplora.cat/prop/>\nPREFIX geo: <http://www.opengis.net/ont/geosparql#>\nSELECT ?species ?photo ?sound ?location WHERE {\n  ?s dwc:scientificName ?species ; p:preview ?photo ; geo:asWKT ?location .\n  ?r dwc:scientificName ?species ; p:audio ?sound .\n} LIMIT 24"},
