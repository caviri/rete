...
window.RETE_PLAYGROUND_CATALOG = {
  ...
  examples: {
    ...
    "orcid": [
      ...
      {"family": "Select", "label": "🔗 Cross-source JOIN: CERN & EPFL people ⋈ ROR org cards", "view": "table", "fed": ["ror"], "cols": {"person": "ORCID", "name": "Researcher", "role": "Role", "orgName": "Organization (ror.rete)", "city": "City (ror.rete)", "country": "Country (ror.rete)"},
       "tip": "The real federation: ONE query split across TWO files and JOINED on the shared https://ror.org/{id} node. orcid.rete (1.3B triples) answers who lists CERN / EPFL and as what; ror.rete answers what those orgs ARE — label, city, country — because ORCID's disambiguation kept ROR's canonical IRIs.\nBoth sides stay selective: a VALUES seed on two orgs, each resolved over HTTP range. Remove the ror chip to see the org columns go unanswered.",
       "q": "PREFIX orc: <https://w3id.org/rete/orcid#>\nPREFIX ror: <https://w3id.org/rete/ror#>\nPREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>\nSELECT ?person ?name ?role ?orgName ?city ?country WHERE {\n  VALUES ?org { <https://ror.org/01ggx4157> <https://ror.org/02s376052> }\n  ?a orc:affiliationWith ?org ;\n     orc:affiliationOf ?person ;\n     orc:roleTitle ?role .\n  ?person rdfs:label ?name .\n  ?org rdfs:label ?orgName ;\n       ror:locationName ?city ;\n       ror:countryName ?country .\n} LIMIT 60"}
    ],
    "ror": [
      ...
      {"family": "Select", "label": "🔗 Cross-source JOIN: who lists EPFL? (live from the 1.3B ORCID graph)", "view": "table", "fed": ["orcid"], "cols": {"orgName": "Organization", "city": "City", "person": "ORCID (orcid.rete)", "name": "Researcher (orcid.rete)", "role": "Role (orcid.rete)", "start": "Since (orcid.rete)"},
       "tip": "The join in the other direction: ror.rete resolves https://ror.org/02s376052 to EPFL (École Polytechnique Fédérale de Lausanne, Lausanne); the 17.5 GB orcid.rete — preloaded as a second source — answers who lists it, in what role, since when.\nA bound org keeps the billion-triple side to a few range reads. Swap the ROR id to poll any institution's people.",
       "q": "PREFIX orc: <https://w3id.org/rete/orcid#>\nPREFIX ror: <https://w3id.org/rete/ror#>\nPREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>\nSELECT ?orgName ?city ?person ?name ?role ?start WHERE {\n  VALUES ?org { <https://ror.org/02s376052> }\n  ?org rdfs:label ?orgName ;\n       ror:locationName ?city .\n  ?a orc:affiliationWith ?org ;\n     orc:affiliationOf ?person ;\n     orc:roleTitle ?role ;\n     orc:startYear ?start .\n  ?person rdfs:label ?name .\n} LIMIT 50"}
    ],
    "zenodo-records": [
