...
;(function(){
  ...
  // A wasm trap ("unreachable") or an allocation/memory error — almost always the
  // browser hitting its WebAssembly memory limit on a heavy query over a large
  // remote file. Common on iPhone/iPad Safari, which caps per-tab memory well
  // below desktop.
  ...
  function runQuery() {
    ...
    if (state.remote) {
      ...
      remoteSparql(state.remote.url, q, remoteFmt).then((out) => {
        ...
        } else if (isEngineMemoryError(msg)) {
          // The wasm instance trapped (out of memory). Rebuild a fresh worker so
          // the next query works, and explain it — don't dump a raw wasm stack.
          resetRemoteWorker();
          $("qmeta").textContent = "out of memory";
          const meta = (CATALOG.datasetMeta && CATALOG.datasetMeta[state.dataset]) || {};
          const sizeNote = meta.size ? ` (${esc(meta.size)})` : "";
          $("out").innerHTML =
            `<div class="note"><b>This query ran out of memory.</b> It needed more than the browser allows for a query over this remote dataset${sizeNote}. ` +
