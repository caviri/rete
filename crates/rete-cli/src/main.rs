...
use rete_core::{
    build_pyramid_meta, eval_bgp, eval_query, write_dataset, CountingReader, DictionaryBuilder,
    GraphIndexBuilder, PatternTerm, QueryOutput, RangeReader, Rete, SliceReader, SummaryView,
    TriplePattern, CODEC_ZSTD, DEFAULT_TILE_BUDGET,
};
...
fn reason_cmd(file: &str, materialize: bool, format: &str) -> anyhow::Result<()> {
    ...
    if materialize {
        ...
        match format {
            "ttl" => print!("{}", commands::export::export_turtle(&all)),
            _ => {
                for (s, p, o) in &all {
                    println!("{s} {p} {o} .");
                }
...
mod tests {
    ...
    fn turtle_export_groups_and_abbreviates() {
        let ttl = commands::export::export_turtle(&sample_triples());
    ...
    fn jsonld_export_expanded_shape() {
        let v: serde_json::Value =
            serde_json::from_str(&commands::export::export_jsonld(&sample_triples())).unwrap();
        let node = &v[0];
